...
<template>
  <div class="box-border flex h-[68px] w-[440px] max-w-full min-w-0 items-center gap-3">
    ...
    <span v-else class="h-9 w-[3px] shrink-0 rounded-[3px] bg-[var(--primary)]" />
    <div class="min-w-0 flex-1">
      <MarqueeText class="font-bold text-[var(--primary)]" :text="media.title" :enabled="settings.marqueeEnabled" />
      <MarqueeText class="mt-[3px] text-xs text-[var(--secondary)]" :text="media.artists.join(', ')" :enabled="settings.marqueeEnabled" />
    </div>
  </div>
</template>

...
<template>
  <div class="box-border flex min-h-24 w-[300px] max-w-full items-center gap-3.5">
    ...
    <div class="min-w-0 text-base font-bold">
      <MarqueeText class="text-[var(--primary)]" :text="media.title" :enabled="settings.marqueeEnabled" />
      <MarqueeText class="mt-[5px] text-xs font-medium text-[var(--secondary)]" :text="media.artists.join(', ')" :enabled="settings.marqueeEnabled" />
    </div>
  </div>
</template>
